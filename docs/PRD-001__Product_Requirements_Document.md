...
Two straightforward approaches are inadequate:
...
| G2 | Keep every viewer's rendering workload small and constant regardless of site size. | K3, K6 (building scale) |
...
| FR-TD-04 | Each device shall name its own metrics with a label unique within that device; each metric instance shall declare its unit, noise threshold, freshness timeout, and, if any, the aggregate channels of its containing group that it contributes to. | M | G3, K5 |
| FR-TD-05 | Each group shall define its own set of aggregate channels — at most 16 per group, in addition to the group's attention value (FR-AG-06). Each channel shall have an identity unique within its group, a unit, an aggregation reduction from a fixed set (sum, mean, min, max, count), and a noise threshold, and may receive contributions from multiple metrics of its unit and/or from its child groups' channels. | M | G3 |
| FR-TD-06 | Each group's aggregate channels shall reflect exactly the contributions configured for it; a group contributes its configured channels to its parent only where configured, while its attention always propagates to its parent (FR-AG-06). Channel contributions shall join only a channel of the same reduction. | M | G3 |
| FR-TD-07 | The system shall validate the topology at boot as a single-rooted, acyclic tree in which every node has a topology-unique identity and every device is a reachable leaf. Groups may be empty. It shall refuse to serve on structural errors and report the fault with enough detail to locate it. | M | G5, R1, R9 |
| FR-TD-08 | The system shall validate the topology and its configuration at boot and refuse to serve on violation, reporting the fault with enough detail to locate it. The validation conditions are listed below. | M | G5, R1, R9 |
| FR-TD-09 | The topology definition shall be the single source shared by the simulator, the engine, and the client, so that identities are reproducible from an unchanged topology. | M | G5 |
| FR-TD-10 | The system shall serve per-viewer visibility queries and compute per-group aggregates, becoming ready to serve within the boot-time budget (K8) and meeting the per-frame latency budgets (K2, K9). | M | G2, G3, K2, K9 |
| FR-TD-11 | The topology shall allow severity limits to be assigned to metrics and channels, mapping a value that reaches or exceeds a high limit, or reaches or falls below a low limit, to a severity level on the fixed severity scale (FR-AG-06); a value's level is the greatest level among its fired limits. An absence severity level may be assigned to each metric for use while that metric is unavailable; a metric without an assigned absence level is treated as normal. Limits inherit the unit of the value they bound. | M | G2, G3 |

Validation conditions for FR-TD-08:
- Metric labels are unique within each device, and each metric instance declares exactly one unit.
- Every device position is finite.
- Each group defines at most 16 configured channels, whose identities are unique within the group.
- Every configured contribution target exists: a metric's channels in its containing group, a channel's in its parent.
- No channel combines contributors of different units, and no channel joins channel contributions of a different reduction.
- No configured contribution addresses a group's attention value.
- Every severity limit has a finite threshold and a level on the fixed severity scale, and every absence severity level lies on that scale.
...
| FR-IS-02 | The system shall discard invalid readings — those naming an unknown device or metric label, or carrying a value that is not a finite number — without disturbing existing values, and shall make each accepted reading visible to the next frame. Invalid readings shall not refresh a device's liveness. | M | G1, G3 |
...
| FR-IS-04 | The system shall report a device offline when all of its metrics are unavailable, and return it online when a metric reading is next accepted; viewers shall be able to distinguish offline devices and unavailable metrics from live ones. | M | G3 |
...
| ID | Requirement | Priority | Source |
| --- | --- | --- | --- |
| FR-AG-01 | For every group and aggregate channel, the system shall compute one combined value that exactly reflects the channel's configured reduction applied to the group's available contributing readings and its configured child-channel contributions; a `mean` is exact over the group's available subtree readings, and a `count` is the total number of available contributing readings in the subtree, merged from child `count` channels by summation. | M | G3 |
| FR-AG-02 | An aggregate channel shall combine only contributions of its own unit and never combine values of different units. | M | G3, R3 |
...
| FR-AG-04 | A group's channel shall include only available contributions; unavailable (stale or offline) readings and valueless child channels shall be excluded. A channel with no available contribution shall report no value rather than a value derived from stale or absent data. | M | G3 |
| FR-AG-05 | The aggregate published for a frame shall include every reading accepted before that frame's aggregation began; a reading accepted during the frame may be included in that frame or deferred to the next. | M | G3 |
| FR-AG-06 | Each group shall have an attention value on the fixed severity scale of normal, advisory, warning, and critical. A metric contributes its limit level when available and its absence severity level when unavailable; a configured channel contributes its limit level when it has a value. A device's attention value is the greatest of its metrics' levels; a group's attention value is the greatest of its configured channels' limit levels, its direct devices' attention values, and its child groups' attention values. Unavailable values contribute no level. Attention propagates to a group's parent independently of the configured contributions and cannot be addressed by configuration. A group with no available contributions has attention value normal. | M | G2, G3 |

### 6.4 Visibility and Selection

| ID | Requirement | Priority | Source |
| --- | --- | --- | --- |
| FR-VS-01 | The system shall accept each viewer's camera pose — position and orientation — and screen geometry (width, height, pixel ratio, field of view) every frame. | M | G2 |
...
| FR-VS-05 | When a group is too small on screen to show its individual devices, the system shall show the group's aggregate values instead; the size threshold and its hysteresis are configured per FR-VS-06. | M | G2 |
...
| FR-TR-02 | For every visible entry, the system shall define the entry's complete value set — a device instance's metrics or a group entry's channels — so the client can resolve any of them locally, and shall send each viewer only the values that changed since those last sent to that viewer, suppressing changes below the applicable noise threshold (each metric's for a device instance, each channel's for a group entry), so that steady-state egress stays within K5 while each displayed value remains within its threshold of the current value. An entry that becomes visible to a viewer for the first time shall be transmitted with its complete value set. Availability changes, attention changes, and a viewer's selected object (FR-VS-02) shall be transmitted regardless of the threshold. | M | G4, K5 |
...
| ID | Requirement | Priority | Source |
| --- | --- | --- | --- |
| FR-CD-01 | The browser client shall render the visible device instances and blended group entries as a live 3D scene that distinguishes unavailable/offline devices from live ones, without rendering stalls. | M | G2, K6, K7 |
| FR-CD-02 | The client shall render at a stable 60 FPS with main-thread overhead < 3 ms per frame. | M | G2, K6 |
| FR-CD-03 | The client shall colour each rendered object by one selected value — a metric for a device instance, a channel for a group entry, or the object's attention value — defaulting to the attention value on the fixed severity scale, and allowing the viewer to change the selection. The attention default shall provide consistent comparison across all rendered objects; selection of a specific metric or channel is per-object inspection. | M | G2, K6 |
...
| FR-CD-05 | While an object is selected, the client shall display its current values and their units plus its availability status, refreshing every frame regardless of how little they change, and shall return to normal streaming when deselected. | M | G2 |
| FR-CD-06 | The client shall publish its camera pose (position and orientation) and screen geometry every frame. | M | G2 |
...
| Streamed updates | engine → browser, every frame | Changes, availability, and snapshots for visible device instances and blended group entries. Every entry's complete value set is defined (a device instance's metrics or a group entry's channels), plus the entry's attention value; transmission follows FR-TR-02 — the full value set on first send or snapshot, changes thereafter. |
...
- Alerting, rule engines, anomaly detection, or stored warning/event history; the engine signals only live device state, including offline. The attention value of FR-AG-06 is in scope: it is derived live from configured severity limits and per-metric absence levels.
- Fraction-based data-coverage policy; absence is represented by the per-metric absence severity levels of FR-AG-06.
- Explicit device out-of-service or offline signalling; a device is offline only when its metrics time out (FR-IS-03, FR-IS-04).
...
| R8 | Schedule slippage | A fixed cut-down ladder, defined before execution; scope-reducing rungs require an explicit, versioned amendment to this document |
| R9 | Misconfigured topology or channels reach serving | Boot validation refuses to serve on structural, channel-count, channel/unit, reduction, or severity-limit errors (FR-TD-07, FR-TD-08) |
